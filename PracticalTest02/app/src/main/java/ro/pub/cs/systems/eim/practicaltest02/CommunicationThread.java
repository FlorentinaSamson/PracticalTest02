...
public class CommunicationThread extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            HashMap<String, CurrencyExchange> data = serverThread.getData();
            CurrencyExchange exchange = null;

            if (data.containsKey(currency)) {
                long diff = new Date().getTime() - data.get(currency).getUpdate().getTime();
                if (diff <  60000)
                    exchange = data.get(currency);
            }
            ...
            if(exchange == null) {
                ...
                switch (currency) {
                    case "EUR":
                        ...
                        exchange = new CurrencyExchange("EUR", eur.getString("rate"));
                        break;
                    case "USD":
                        JSONObject usd = bpi.getJSONObject("USD");
                        exchange = new CurrencyExchange("USD", usd.getString("rate"));
                        break;
                    default:
                        result = "[COMMUNICATION THREAD] Wrong currency";
                }

                serverThread.setData(currency, exchange);
            }

            printWriter.println(exchange.getExchangeRate());
            printWriter.flush();
